...
{ 
	...
	uint256 giveTime = datetime.toTimestamp(2022,11,17,0,0);
	string faultName = "" ;
	ufixed amount = 0 ;
	...
	function delivery1() internal view returns (bool) {
		if (isDone("rule2")&&!isTime(functionFinishTime["rule2"]+60*60*24*1)
		) return true;
		else return false;
	}
	function delivery2() internal view returns (bool) {
		if (isDone("rule2_2")&&!isTime(functionFinishTime["rule2_2"]+60*60*24*1)
		) return true;
		else return false;
	}
