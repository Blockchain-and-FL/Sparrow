...
contract BuyerT{
	...
	//attributes of actionBuyerSetResult
	bool _isBuyerSetResultDone;
	uint _BuyerSetResultTime;
	...
	function BuyerSetResultDone(){
		_BuyerSetResultTime = now;
		_isBuyerSetResultDone = true;
	}
	
	function BuyerSetResultTime() returns (uint result){
	    if(_isBuyerSetResultDone){
	        return _BuyerSetResultTime;
	    }
	    return _max;
	}
